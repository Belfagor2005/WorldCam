...
#        os.mkfifo(STREAM_PFILE)
        ...
        if len(variants) == 1:
            if PY3:
                self.url = urllib.parse.urljoin(self.url, variants[0][0])
            else:
...
##                        self.stop()
#                print
            choice = max(iter(autoChoice.items()), key=operator.itemgetter(1))[0]
            pass#print '[hlsclient::play] Autoselecting %s' % choice
            #Use the first choice for testing
##            choice = 0
            self.url = urllib.parse.urljoin(self.url, variants[choice][0])
        ...
        while self.thread.isAlive():
                ...
                for media in medialist:
                  try:
                    ...
                    if seq > last_seq:
                        if PY3:
                            for chunk in self.download_chunks(urllib.parse.urljoin(self.url, media_url)):
                                if enc:
