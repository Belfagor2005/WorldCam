...
class RokfinIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://www.rokfin.com/post/57548/Mitt-Romneys-Crazy-Solution-To-Climate-Change',
               'info_dict': {'id': 'post/57548',
                             'ext': 'mp4',
    ...
    def _real_extract(self, url):
        ...
        timestamp = (
            scheduled or float_or_none(
                metadata.get('postedAtMilli'),
                1000) or unified_timestamp(
                metadata.get('creationDateTime')))
        return {
            'id': video_id,
            'formats': formats,
            'subtitles': subtitles,
    ...
    def _get_comments(self, video_id):
        ...
        for page_n in itertools.count():
            ...
            if not raw_comments.get('content') or is_last or (
    ...
    def _perform_login(self, username, password):
        ...
        if not self._authentication_active():
            if re.search(
                r'(?i)(invalid\s+username\s+or\s+password)',
                    resp_body or ''):
    ...
    def _authentication_active(self):
        return not ({'KEYCLOAK_IDENTITY',
                     'KEYCLOAK_IDENTITY_LEGACY',
                     'KEYCLOAK_SESSION',
                     'KEYCLOAK_SESSION_LEGACY'} - set(self._get_cookies(self._AUTH_BASE)))

    def _get_auth_token(self):
        return try_get(self._access_mgmt_tokens, lambda x: ' '.join(
    ...
    def _get_video_data(self, metadata):
        for content in metadata.get('content') or []:
            ...
            video_id = content.get(
                'id') if media_type == 'post' else content.get('mediaId')
...
class RokfinChannelIE(RokfinPlaylistBaseIE):
    ...
    def _validate_extractor_args(self):
        ...
        if requested_tabs is not None and (
            ...
            raise ExtractorError(
                f'Invalid extractor-arg "tab". Must be one of {", ".join(self._TABS)}', expected=True)
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(self._entries(channel_id,
                                                  channel_name,
                                                  self._TABS[tab]),
                                    f'{channel_id}-{tab}',
