...
class QDanceIE(InfoExtractor):
    ...
    _TESTS = [{'note': 'vod',
              ...
              {'note': 'vod with alphanumeric id',
               ...
               'info_dict': {'id': 'WhDleSIWSfeT3Q9ObBKBeA',
                             'ext': 'mp4',
    ...
    def _get_auth(self):
        if (try_call(lambda: jwt_decode_hs256(self._access_token)
    ...
    def _real_extract(self, url):
        ...
        def extract_availability(level):
            ...
            return self._availability(
                needs_premium=(
                    level >= 20), needs_subscription=(
                    level >= 15), needs_auth=True)
        ...
        if not m3u8_url and traverse_obj(
                stream, ('error', 'code')) == 'unauthorized':
            raise ExtractorError(
