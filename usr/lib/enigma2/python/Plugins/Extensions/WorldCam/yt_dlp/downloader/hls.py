...
class HlsFD(FragmentFD):
    ...
    def _has_drm(manifest):  # TODO: https://github.com/yt-dlp/yt-dlp/pull/5039
        return bool(re.search('|'.join((
            ...
            # Apple FairPlay
            r'#EXT-X-(?:SESSION-)?KEY:.*?KEYFORMAT="com\.apple\.streamingkeydelivery"',
            # Microsoft PlayReady
            r'#EXT-X-(?:SESSION-)?KEY:.*?KEYFORMAT="com\.microsoft\.playready"',
    ...
    def can_download(cls, manifest, info_dict, allow_unplayable_formats=False):
        ...
        if not allow_unplayable_formats:
            UNSUPPORTED_FEATURES += [
                # encrypted streams [1], but not necessarily DRM
    ...
    def real_download(self, filename, info_dict):
        ...
        can_download, message = self.can_download(
            s, info_dict, self.params.get('allow_unplayable_formats')), None
        if can_download:
            has_ffmpeg = FFmpegFD.available()
            if not Cryptodome.AES and '#EXT-X-KEY:METHOD=AES-128' in s:
                # Even if pycryptodomex isn't available, force HlsFD for m3u8s
                # that won't work with ffmpeg
        ...
        if is_webvtt:
            # Packing the fragments is not currently supported for external
            # downloader
        ...
        if external_aes_key:
            ...
            assert len(external_aes_key) in (
                16, 24, 32), 'Invalid length for HLS AES-128 key'
        ...
        for line in s.splitlines():
            ...
            if line:
                if not line.startswith('#'):
                    ...
                    # If the byte_range is truthy, reset it after appending a
                    ...
                    if map_info.get('BYTERANGE'):
                        splitted_byte_range = map_info.get(
                            'BYTERANGE').split('@')
                        sub_range_start = int(splitted_byte_range[1]) if len(
                            splitted_byte_range) == 2 else 0
                        map_byte_range = {
                    ...
                    if decrypt_info['METHOD'] == 'AES-128':
                        ...
                        if external_aes_key:
                            ...
                            if extra_key_query or extra_segment_query:
                                # Fall back to extra_segment_query to key for
                                # backwards compat
            ...
            def pack_fragment(frag_content, frag_index):
                ...
                for block in webvtt.parse_fragment(frag_content):
                    if isinstance(block, webvtt.CueBlock):
                        ...
                        # we only emit cues once they fall out of the duplicate
                        # window
                        continue
