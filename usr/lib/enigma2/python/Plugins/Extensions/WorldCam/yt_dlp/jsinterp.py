...
def js_number_to_string(val: float, radix: int = 10):
    ...
    assert radix in range(
        2, 37), 'radix must be an integer at least 2 and no greater than 36'
    ...
    while fraction >= delta:
        ...
        # if we need to round, propagate potential carry through fractional
        # part
        ...
        def interpret_statement(
            ...
            if cls.ENABLED and stmt.strip():
                if should_ret or repr(ret) != stmt:
                    cls.write(['->', '=>'][should_ret], repr(ret),
                              '<-|', stmt, level=allow_recursion)
...
class JSInterpreter:
    ...
    def _separate(expr, delim=',', max_split=None):
        ...
        for idx, char in enumerate(expr):
            ...
            after_op = char if (
                not in_quote and char in OP_CHARS) else (
                char.isspace() and after_op)

            if char != delim[pos] or any(
                    counters.values()) or in_quote or in_unary_op:
                pos = 0
                continue
            elif pos != delim_len:
                pos += 1
    ...
    def interpret_statement(
        ...
        if expr.startswith('new '):
            ...
            if obj.startswith('Date('):
                ...
                date = unified_timestamp(
                    self.interpret_expression(
                        left, local_vars, allow_recursion), False)
                if date is None:
                    raise self.Exception(
            ...
            if all(len(sub_expr) == 2 for sub_expr in sub_expressions):
                def dict_item(key, val):
                    ...
                    return self.interpret_expression(
                        key, local_vars, allow_recursion), val

                return dict(dict_item(k, v)
                            for k, v in sub_expressions), should_return
        ...
        if m:  # We are assigning a value to a variable
            ...
            if not m.group('index'):
                ...
                raise self.Exception(
                    f'Cannot index undefined variable {
                        m.group("out")}', expr)
        ...
        for op in _OPERATORS:
            ...
            while True:
                if op in '?<>*-' and len(
                        separated) > 1 and not separated[-1].strip():
                    separated.pop()
            ...
            return self._operator(
                op, left_val, right_expr, expr, local_vars, allow_recursion), should_return
        ...
        if m and m.group('attribute'):
            ...
            def eval_method():
                ...
                if (variable, member) == ('console', 'debug'):
                    if Debugger.ENABLED:
                        Debugger.write(self.interpret_expression(
