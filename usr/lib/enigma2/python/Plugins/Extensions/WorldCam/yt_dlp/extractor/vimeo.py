...
class VimeoBaseInfoExtractor(InfoExtractor):
    ...
    def _get_requested_client(self):
        if client := self._configuration_arg(
                'client', [None], ie_key=VimeoIE)[0]:
            if client not in self._CLIENT_CONFIGS:
                raise ExtractorError(
                    f'Unsupported API client "{client}" requested. ' f'Supported clients are: {
                        ", ".join(
    ...
    def _call_videos_api(
        ...
        if client_config['REQUIRES_AUTH'] and not self._is_logged_in:
            self.raise_login_required(
                f'The {client} client only works when logged-in')
    ...
    def _extract_original_format(self, url, video_id, unlisted_hash=None):
        ...
        policy = self._configuration_arg(
            'original_format_policy', ['auto'], ie_key=VimeoIE)[0]
        if policy == 'never':
            return None
        ...
        try:
            ...
            self.write_debug(
                f'Unable to load download config JSON: {
                    error.cause}')
            download_data = None

        source_file = traverse_obj(
            download_data, ('source_file', {dict})) or {}
        download_url = traverse_obj(
            source_file, ('download_url', {url_or_none}))
        ...
        # so first we should check if video even has 'download' formats
        # available
        try:
            privacy_info = self._call_videos_api(
                video_id, unlisted_hash, force_client='web', query={
                    'fields': 'privacy'})
        except ExtractorError as error:
...
class VimeoIE(VimeoBaseInfoExtractor):
    ...
    _TESTS = [{
        ...
        # requires passing unlisted_hash(a52724358e) to load_download_config
        # request
        ...
        # chapters must be sorted, see:
        # https://github.com/yt-dlp/yt-dlp/issues/5308
                ...
                response = traverse_obj(
                    self._webpage_read_content(
                        e.cause.response, e.cause.response.url, video_id, fatal=False), ({
                            json.loads}, {dict})) or {}
                if (not retry and e.cause.status == 400 and 'password' in traverse_obj(
                        response, ('invalid_parameters', ..., 'field'))):
    ...
    def _real_extract(self, url):
        ...
        album_id = self._search_regex(
            r'vimeo\.com/(?:album|showcase)/([0-9]+)/',
            url,
            'album id',
            default=None)
        if album_id:
            # Detect password-protected showcase video => POST album password
            # => set cookies
...
class VimeoAlbumIE(VimeoBaseInfoExtractor):
    ...
    def _fetch_page(self, album_id, hashed_pass, is_embed, referer, page):
        ...
        for video in videos:
            ...
            video_id = self._search_regex(
                r'/videos/(\d+)', uri, 'id', default=None) if uri else None
            if is_embed:
                if not video_id:
                    self.report_warning(
                        f'Skipping due to missing video ID: {link}')
                    continue
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if is_embed and not referer and isinstance(
    ...
    def _real_extract(self, url):
        ...
        if clip_status == 'unavailable' and release_timestamp and release_timestamp > time.time():
            ...
            if video_id:
                ...
                def video_filter(
                    _, v): return traverse_obj(
                    v, ('live', 'status')) != 'unavailable'
