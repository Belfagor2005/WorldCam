...
class NFBIE(NFBBaseIE):
    ...
    _TESTS = [{'note': 'NFB film',
               'url': 'https://www.nfb.ca/film/trafficopter/',
               'info_dict': {'id': 'trafficopter',
                             'ext': 'mp4',
                             'title': 'Trafficopter',
              ...
              {'note': 'NFB episode with English title',
               ...
               'info_dict': {'id': 'true-north-episode9-true-north-finale-making-it',
                             'ext': 'mp4',
              ...
              {'note': 'ONF episode with French title',
               ...
               'info_dict': {'id': 'direction-nord-episode-9',
                             'ext': 'mp4',
              ...
              {'note': 'NFB episode with French title (needs geo-bypass)',
               ...
               'info_dict': {'id': 'etoile-du-nord-episode-1-lobservation',
                             'ext': 'mp4',
              ...
              {'note': 'ONF episode with English title (needs geo-bypass)',
               ...
               'info_dict': {'id': 'north-star-episode-1-observation',
                             'ext': 'mp4',
              ...
              {'note': 'Season 2 episode w/o episode num in id, extract from json ld',
               ...
               'info_dict': {'id': 'liste-des-choses-qui-existent-saison-2-ours',
                             'ext': 'mp4',
    ...
    def _real_extract(self, url):
        site, type_, slug = self._match_valid_url(
            url).group('site', 'type', 'id')
        # Need to construct the URL since we match /embed/player/ URLs as well
        webpage, urlh = self._download_webpage_handle(
            f'https://www.{site}.ca/{type_}/{slug}/', slug)
        # type_ can change from film to serie(s) after redirect; new slug may
        # have episode number
        ...
        video_id = self._match_id(player_data['overlay']['url'])
        ...
        info = {
            ...
            'release_year': int_or_none(
                self._html_search_regex(
                    ...
                    'release_year',
                    default=None)),
        } if type_ == 'film' else self._extract_ep_info(
            self._extract_ep_data(
                webpage,
                video_id,
                slug),
            video_id)
...
class NFBSeriesIE(NFBBaseIE):
    ...
    def _entries(self, episodes):
        for episode in traverse_obj(
            episodes, lambda _, v: NFBIE.suitable(
                v['embed_url'])):
    ...
    def _real_extract(self, url):
        site, type_, series_id = self._match_valid_url(
            url).group('site', 'type', 'id')
