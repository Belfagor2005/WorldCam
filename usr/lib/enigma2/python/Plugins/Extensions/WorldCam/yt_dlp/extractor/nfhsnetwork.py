...
class NFHSNetworkIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        broadcast = (publisher.get('broadcasts') or publisher.get('vods'))[
            0]  # some (older) videos don't have a broadcasts object
        ...
        is_live = broadcast.get('on_air') or broadcast.get(
            'status') == 'on_air' or False
        ...
        key = broadcast.get('key') if is_live else try_get(
            publisher, lambda x: x['vods'][0]['key'])
