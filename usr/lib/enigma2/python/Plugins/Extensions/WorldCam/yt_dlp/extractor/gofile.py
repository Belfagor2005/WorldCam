...
class GofileIE(InfoExtractor):
    ...
    def _entries(self, file_id):
        ...
        if status == 'error-passwordRequired':
            ...
            raise ExtractorError(
                f'{self.IE_NAME} said: status {status}',
                expected=True)
        ...
        if not found_files:
            raise ExtractorError(
                'No video/audio found at provided URL.',
                expected=True)
