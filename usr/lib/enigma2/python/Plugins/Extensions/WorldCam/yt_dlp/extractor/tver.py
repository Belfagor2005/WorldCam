...
class TVerIE(StreaksBaseIE):
    ...
    def _real_extract(self, url):
        ...
        metadata = {
            ...
            'series': series,
            'episode': episode,
            # an another title which is considered "full title" for some
            # viewers
        ...
        # Deprecated Brightcove extraction reachable w/extractor-arg or
        # fallback; errors are expected
        if backend == 'brightcove' or not streaks_id:
            ...
            if not brightcove_id:
                raise ExtractorError(
                    'Unable to extract brightcove reference ID',
                    expected=True)
            account_id = traverse_obj(video_info, ('video', 'accountID', {str}, {
        ...
        key_idx = dt.datetime.fromtimestamp(
            time_seconds(hours=9), dt.timezone.utc).month % 6 or 6
        ...
        try:
            ...
            # Catch and re-raise with metadata_available to support
        ...
        return {
            ...
            **metadata,
            ...
            '_old_archive_ids': [
                make_archive_id(
                    'BrightcoveNew',
                    brightcove_id)] if brightcove_id else None,
        }
