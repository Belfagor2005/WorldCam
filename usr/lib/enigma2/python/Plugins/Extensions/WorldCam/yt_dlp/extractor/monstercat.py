...
class MonstercatIE(InfoExtractor):
    ...
    def _extract_tracks(self, table, album_meta):
        for td in re.findall(
            r'<tr[^<]*>((?:(?!</tr>)[\w\W])+)',
                table):  # regex by chatgpt due to lack of get_elements_by_tag
            ...
            ids = traverse_obj(
                td, ({
                    find_element(
                        cls='btn-play cursor-pointer mr-small', html=True)}, {extract_attributes})) or {}
    ...
    def _real_extract(self, url):
        ...
        # NB: HTMLParser may choke on this html; use {find_element} or
        # try_call(lambda: get_element...)
        tracklist_table = traverse_obj(
            html, {find_element(cls='table table-small')}) or ''
        ...
        album_meta = {'title': title,
                      ...
                      'album_artists': traverse_obj(html,
                                                    ({find_element(cls='h-normal text-uppercase mb-desktop-medium mb-smallish')},
                                                     {clean_html},
                                                     all)),
