...
class EmbedThumbnailPP(FFmpegPostProcessor):
    ...
    def _get_thumbnail_resolution(self, filename, thumbnail_dict):
        def guess():
            width, height = thumbnail_dict.get(
                'width'), thumbnail_dict.get('height')
            if width and height:
                return width, height
    ...
    def run(self, info):
        ...
        idx = next((-i for i,
                    t in enumerate(info['thumbnails'][::-1],
                                   1) if t.get('filepath')),
                   None)
        if idx is None:
        ...
        if info['ext'] not in (
                'mkv', 'mka') and thumbnail_ext not in (
                'jpg', 'jpeg', 'png'):
        ...
        if info['ext'] == 'mp3':
            ...
            if not mutagen or prefer_atomicparsley:
                ...
                try:
                    ...
                    # Apple iTunes 'M4A' files include the
            ...
            if not success:
                success = True
                atomicparsley = next((
                    # libatomicparsley.so : See
                    # https://github.com/xibr/ytdlp-lazy/issues/1
            ...
            # See https://github.com/yt-dlp/yt-dlp/issues/2125,
            # https://github.com/yt-dlp/yt-dlp/issues/411
            if not success:
                success = True
                try:
