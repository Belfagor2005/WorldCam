    ...
    def _lock_file(f, exclusive, block):
        ...
        if not LockFileEx(msvcrt.get_osfhandle(f.fileno()),
            ...
            raise BlockingIOError(
                'Locking file failed: {0!r}'.format(
...
class RetryManager:
    ...
    def report_retry(e, count, retries, *, sleep_func, info, warn, error=None, suffix=None):
        ...
        if count > retries:
            if error:
                return error(
                    '{0}. Giving up after {1} retries'.format(e, count - 1)
                ) if count > 1 else error(str(e))
            raise e
        ...
        warn(
            '{0}. Retrying{1} ({2}/{3})...'.format(
                e,
                format_field(suffix, None, " %s"),
                count,
                retries
            )
        )
    ...
    def _get_field_setting(self, field, key):
        if field not in self.settings:
            ...
            self.ydl.deprecated_feature(
                'Using arbitrary fields ({0}) for format sorting is deprecated and may be removed in a future version'.format(field)
            )
            self.settings[field] = {}
        ...
        for item in sort_list:
            ...
            if self._get_field_setting(field, 'type') == 'alias':
                ...
                if self._get_field_setting(alias, 'deprecated'):
                    self.ydl.deprecated_feature(
                        'Format sorting alias {0} is deprecated and may be removed in a future version. Please use {1} instead'.format(
                            alias, field
                        )
                    )
