...
class HolodexIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://holodex.net/watch/9kQ2GtvDV3s',
    ...
    def _real_extract(self, url):
        video_id, playlist_id, pl_id2 = self._match_valid_url(
            url).group('id', 'playlist', 'playlist2')
        ...
        data = self._download_json(
            f'https://holodex.net/api/v2/playlist/{playlist_id}',
            playlist_id)
        return self.playlist_from_matches(traverse_obj(
            data, ('videos', ..., 'id')), playlist_id, data.get('name'), ie=YoutubeIE)
