...
class PatreonBaseIE(InfoExtractor):
    ...
    def _call_api(
        ...
        try:
            ...
            if not isinstance(e.cause, HTTPError) or mimetype2ext(
                    e.cause.response.headers.get('Content-Type')) != 'json':
                raise
...
class PatreonIE(PatreonBaseIE):
    ...
    _TESTS = [{
        ...
        # bad videos under media (if media is included). Real one is under
        # post_file
    ...
    def _real_extract(self, url):
        ...
        for include in traverse_obj(
            ...
            if include_type == 'media':
                media_attributes = traverse_obj(
                    include, ('attributes', {dict})) or {}
                ...
                info.update(
                    traverse_obj(
                        include, {
                            'channel': (
                                'attributes', 'title', {str}), 'channel_id': (
                                'id', {str_or_none}), 'channel_url': (
                                'attributes', 'url', {url_or_none}), 'channel_follower_count': (
                                'attributes', 'patron_count', {int_or_none}), }))

        # Must be all-lowercase 'referer' so we can smuggle it to Generic, SproutVideo, and Vimeo.
        # patreon.com URLs redirect to www.patreon.com; this matters when
        # requesting mux.com m3u8s
        ...
        if traverse_obj(attributes, ('embed', 'provider')) == 'Vimeo':
            ...
            if url_or_none(v_url) and self._request_webpage(
                ...
                fatal=False,
                errnote=False,
                    expected_status=429):  # 429 is TLS fingerprint rejection
        ...
        if embed_url and (urlh := self._request_webpage(
            ...
            # to check for "Sorry, we couldn&amp;rsquo;t find that page" in the
            ...
            meta_description = clean_html(
                self._html_search_meta(
                    ...
                    self._webpage_read_content(
                        ...
                        fatal=False),
                    default=None))
            # Password-protected vids.io embeds return 403 errors w/o
            # --video-password or session cookie
            if ((urlh.status != 403 and meta_description !=
                    'Sorry, we couldn’t find that page') or VidsIoIE.suitable(embed_url)):
                entries.append(
                    self.url_result(
                        smuggle_url(
                            embed_url,
...
class PatreonCampaignIE(PatreonBaseIE):
    ...
    _TESTS = [{'url': 'https://www.patreon.com/dissonancepod/',
               'info_dict': {'title': 'Cognitive Dissonance Podcast',
                             'channel_url': 'https://www.patreon.com/dissonancepod',
                             'id': '80642',
              ...
              {'url': 'https://www.patreon.com/m/4767637/posts',
               'info_dict': {'title': 'Not Just Bikes',
                             'id': '4767637',
                             'channel_id': '4767637',
              ...
              {'url': 'https://www.patreon.com/api/campaigns/4243769/posts',
               'info_dict': {'title': 'Second Thought',
                             'channel_follower_count': int,
                             'id': '4243769',
                             'channel_id': '4243769',
    ...
    def _real_extract(self, url):
        ...
        campaign_id, vanity = self._match_valid_url(
            url).group('campaign_id', 'vanity')
        if campaign_id is None:
            webpage = self._download_webpage(
                url, vanity, headers={
                    'User-Agent': self.patreon_user_agent})
