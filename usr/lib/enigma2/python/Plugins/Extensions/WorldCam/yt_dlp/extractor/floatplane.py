...
class FloatplaneBaseIE(InfoExtractor):
    def _real_extract(self, url):
        ...
        uploader_url = format_field(
            post_data, [
                ('creator', 'urlname')], f'{
                self._BASE_URL}/channel/%s/home') or None

        common_info = {
            'uploader_url': uploader_url, 'channel_url': urljoin(
                f'{uploader_url}/', traverse_obj(
                    post_data, ('channel', 'urlname'))), 'availability': self._availability(
                needs_subscription=True), **traverse_obj(
                    post_data, {
                        'uploader': (
                            'creator', 'title', {str}), 'uploader_id': (
                                'creator', 'id', {str}), 'channel': (
                                    'channel', 'title', {str}), 'channel_id': (
                                        'channel', 'id', {str}), 'release_timestamp': (
                                            'releaseDate', {parse_iso8601}), }), }
        ...
        for media in traverse_obj(
            ...
            cdn_base_url = traverse_obj(stream, ('groups', 0, 'origins', ..., 'url', {
                                        url_or_none}, any, {require('cdn base url')}))
            ...
            for variant in traverse_obj(
                ...
                # If we need impersonation for the API, then we need it for HLS
                # keys too: extract in advance
...
class FloatplaneIE(FloatplaneBaseIE):
    ...
    _TESTS = [{'url': 'https://www.floatplane.com/post/2Yf3UedF7C',
               'info_dict': {'id': 'yuleLogLTT',
                             'ext': 'mp4',
              ...
              {'url': 'https://www.floatplane.com/post/65B5PNoBtf',
               ...
               'playlist_count': 2,
               'playlist': [{'info_dict': {'id': 'ISPJjexylS',
                                           'ext': 'mp4',
                                           'release_date': '20231129',
                                           'release_timestamp': 1701249480,
