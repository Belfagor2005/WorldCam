...
class BpbIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'http://www.bpb.de/mediathek/297/joachim-gauck-zu-1989-und-die-erinnerung-an-die-ddr',
               'info_dict': {'id': '297',
                             'ext': 'mp4',
    ...
    def _parse_vue_attributes(self, name, string, video_id):
        attributes = extract_attributes(self._search_regex(
            rf'(<{name}(?:"[^"]*?"|[^>])*>)', string, name))
    ...
    def _real_extract(self, url):
        ...
        title_result = traverse_obj(
            webpage, ({find_element(cls='opening-header__title')}, {self._TITLE_RE.match}))
        json_lds = list(self._yield_json_ld(webpage, video_id, fatal=False))

        return {
            'id': video_id,
            'title': traverse_obj(title_result, ('title', {str.strip})) or None,
            # This metadata could be interpreted otherwise, but it fits
            # "series" the most
