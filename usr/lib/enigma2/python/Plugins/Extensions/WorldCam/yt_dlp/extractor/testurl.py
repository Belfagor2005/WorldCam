...
class TestURLIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        extractor_id, num = self._match_valid_url(
            url).group('extractor', 'num')
        if not extractor_id:
            return {'id': ':test', 'title': '', 'url': url}

        rex = re.compile(extractor_id, flags=re.IGNORECASE)
        matching_extractors = [
            e for e in gen_extractor_classes() if rex.search(
                e.IE_NAME)]

        if len(matching_extractors) == 0:
            raise ExtractorError(
                f'No extractors matching {extractor_id!r} found',
                expected=True)
            ...
            if not extractor:
                raise ExtractorError('Found multiple matching extractors: {}'.format(
                    ' '.join(ie.IE_NAME for ie in matching_extractors)), expected=True)
        else:
