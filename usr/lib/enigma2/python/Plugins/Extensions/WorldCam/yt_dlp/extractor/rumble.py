...
class RumbleEmbedIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://rumble.com/embed/v5pv5f',
              ...
              {'url': 'https://rumble.com/embed/vunh1h',
               'info_dict': {'id': 'vunh1h',
                             ...
                             'uploader': 'BLCKBX',
                             'live_status': 'not_live',
                             'subtitles': {'en': [{'url': r're:https://.+\.vtt',
                                                   'name': 'English',
                                                   'ext': 'vtt',
                                                   },
                                                  ],
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        if embeds:
            return embeds
        return [
            f'https://rumble.com/embed/{
                mobj.group("id")}' for mobj in re.finditer(
    ...
    def _real_extract(self, url):
        ...
        if video.get('live') == 0:
            live_status = 'not_live' if video.get(
                'livestream_has_dvr') is None else 'was_live'
        elif video.get('live') == 1:
            live_status = 'is_upcoming' if video.get(
                'livestream_has_dvr') else 'was_live'
        ...
        for format_type, format_info in (video.get('ua') or {}).items():
            ...
            for video_info in format_info:
                ...
                if not video_info.get('url'):
                    continue
                # With default query params returns m3u8 variants which are
                # duplicates, without returns tar files
                if format_type == 'tar':
                    continue
                if format_type == 'hls':
