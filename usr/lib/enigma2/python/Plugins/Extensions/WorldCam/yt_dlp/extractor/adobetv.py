...
class AdobeTVVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for source in traverse_obj(
            ...
            ('sources',
             lambda _,
             v: v['format'] != 'playlist' and url_or_none(
                 v['src']),
             )):
                    ...
                    traverse_obj(
                        source, {
                            'duration': (
                                ...
                                'kilobytes', {
                                    float_or_none(
                                        invscale=1000)}), 'format_id': (
                                ('format', 'label'), {str}, all, {
                                    lambda x: join_nonempty(
                                        *x)}), 'height': (
                                'height', {int_or_none}), 'tbr': (
                                'bitrate', {int_or_none}), 'width': (
                                'width', {int_or_none}), }))
            formats.extend(fmts)
        ...
        for translation in traverse_obj(video_data, (
            ...
            lang = translation.get('language_w3c') or ISO639Utils.long2short(
                translation.get('language_medium')) or 'und'
            ...
            'id': video_id, 'formats': formats, 'subtitles': subtitles, **traverse_obj(
                video_data, {
                    'title': (
                        ...
                        'description', {clean_html}, filter), 'thumbnail': (
                        'video', 'poster', {
