...
class SapoIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'http://videos.sapo.pt/UBz95kOtiWYUMTA5Ghfi',
               'md5': '79ee523f6ecb9233ac25075dee0eda83',
               'note': 'SD video',
               'info_dict': {'id': 'UBz95kOtiWYUMTA5Ghfi',
                             'ext': 'mp4',
              ...
              {'url': 'http://videos.sapo.pt/IyusNAZ791ZdoCY5H5IF',
               ...
               'note': 'HD video',
               'info_dict': {'id': 'IyusNAZ791ZdoCY5H5IF',
                             'ext': 'mp4',
              ...
              {'url': 'http://v2.videos.sapo.pt/yLqjzPtbTimsn2wWBKHz',
               ...
               'note': 'v2 video',
               'info_dict': {'id': 'yLqjzPtbTimsn2wWBKHz',
                             'ext': 'mp4',
    ...
    def _real_extract(self, url):
        ...
        item = self._download_xml(
            f'http://rd3.videos.sapo.pt/{video_id}/rss2',
            video_id).find('./channel/item')

        title = item.find('./title').text
        description = item.find('./{http://videos.sapo.pt/mrss/}synopse').text
        thumbnail = item.find(
            './{http://search.yahoo.com/mrss/}content').get('url')
        ...
        age_limit = 18 if item.find(
            './{http://videos.sapo.pt/mrss/}m18').text == 'true' else 0

        video_url = item.find('./{http://videos.sapo.pt/mrss/}videoFile').text
        video_size = item.find(
            './{http://videos.sapo.pt/mrss/}videoSize').text.split('x')
