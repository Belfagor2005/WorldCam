...
class NBCUniversalBaseIE(ThePlatformBaseIE):
    ...
    def _download_nbcu_smil_and_extract_m3u8_url(
            self, tp_path, video_id, query):
        ...
        raise ExtractorError(
            traverse_obj(
                smil, (f'{ns}ref/@abstract', ..., any)), expected=exc == 'Expired')
    ...
    def _extract_nbcu_formats_and_subtitles(self, tp_path, video_id, query):
        ...
        # formats='m3u+none,mpeg4' may return DRM HLS but w/the "folders"
        # needed for non-DRM template
        ...
        if mobj := re.fullmatch(self._M3U8_RE, m3u8_url):
            ...
            m3u8_tmpl = self._download_nbcu_smil_and_extract_m3u8_url(
                tp_path, video_id, query)
            # Example:
            # https://vod-lf-oneapp-prd.akamaized.net/prod/video/{folders}master_hls.m3u8
            if '{folders}' in m3u8_tmpl:
                self.write_debug(
                    'Found m3u8 URL template, formatting URL path')
    ...
    def _extract_nbcu_video(self, url, display_id, old_ie_key=None):
        ...
        if tve:
            account_pid = tve.get(
                'data-mpx-media-account-pid') or tve['data-mpx-account-pid']
            ...
            ls_playlist = traverse_obj(
                settings, ('ls_playlist', lambda _, v: v['defaultGuid'], any, {
                    require('LS playlist')}))
            video_id = ls_playlist['defaultGuid']
            account_pid = ls_playlist.get(
                'mpxMediaAccountPid') or ls_playlist['mpxAccountPid']
            account_id = ls_playlist['mpxMediaAccountId']
            metadata = traverse_obj(
                ls_playlist, ('videos', lambda _, v: v['guid'] == video_id, any)) or {}
  ...
  bonanzaPage(
    ...
    metadata {
      ... on VideoPageData {
        ...
        tpm = self._download_theplatform_metadata(
            tp_path, video_id, fatal=False)
        title = traverse_obj(
            tpm, ('title', {str})) or video_data.get('secondaryTitle')
        query = {}
...
class NBCOlympicsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        if event_config.get('cdnToken'):
            ...
            # XXX: The None arg below needs to be the software_statement for
            # this requestor
            media_token = self._extract_mvpd_auth(
                url, pid, event_config.get(
                    'requestorId', 'NBCOlympics'), ap_resource, None)
    ...
    def _real_extract(self, url):
        ...
        if player_id and fw_ssid:
            ...
            if not isinstance(smil, xml.etree.ElementTree.Element):
                smil = None
        subtitles = self._parse_smil_subtitles(
            smil, default_ns) if smil is not None else {}
        for video in smil.findall(
            self._xpath_ns(
                './/video',
                default_ns)) if smil is not None else []:
            info['duration'] = float_or_none(
                remove_end(video.get('dur'), 'ms'), 1000)
            video_src_url = video.get('src')
            ext = mimetype2ext(
                video.get('type'),
                default=determine_ext(video_src_url))
            if ext == 'm3u8':
...
class SyfyIE(NBCUniversalBaseIE):
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        return self._extract_nbcu_video(
            url, display_id, old_ie_key='ThePlatform')
