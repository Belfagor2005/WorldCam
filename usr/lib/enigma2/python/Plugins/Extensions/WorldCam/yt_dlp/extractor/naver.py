...
class NaverBaseIE(InfoExtractor):
    ...
    def process_subtitles(vod_data, process_url):
        ...
        for caption in traverse_obj(vod_data, ('captions', 'list', ...)):
            ...
            if not caption_url:
                continue
            type_ = 'automatic_captions' if caption.get(
                'type') == 'auto' else 'subtitles'
            lang = caption.get('locale') or join_nonempty(
                'language', 'country', from_dict=caption) or 'und'
            if caption.get('type') == 'fan':
                lang += '_fan{}'.format(next(i for i in itertools.count(1)
                ...
                formats.append(
                    {
                        ...
                        'ext': 'mp4',
                        'width': int_or_none(
                            encoding_option.get('width')),
                        'height': int_or_none(
                            encoding_option.get('height')),
                        'vbr': int_or_none(
                            bitrate.get('video')),
                        'abr': int_or_none(
                            bitrate.get('audio')),
                        ...
                        'extra_param_to_segment_url': urllib.parse.urlencode(
                            query,
                            doseq=True) if stream_type == 'HLS' else None,
                    })
    ...
    def _call_api(self, path, video_id):
        ...
        md = base64.b64encode(hmac.HMAC(key,
...
class NaverLiveIE(NaverBaseIE):
    ...
    _GEO_BYPASS = False
    _TESTS = [{'url': 'https://tv.naver.com/l/127062',
               'info_dict': {'id': '127062',
                             'ext': 'mp4',
                             'live_status': 'is_live',
                             'channel': '뉴스는 YTN',
...
class NaverNowIE(NaverBaseIE):
    ...
    def _extract_replay(self, show_id, replay_id):
        ...
        in_key = self._download_json(
            f'{self._API_URL}/shows/now.{show_id}/vod/{replay_id}/inkey', replay_id)['inKey']
        return merge_dicts({
    ...
    def _extract_show_highlights(self, show_id, highlight_id=None):
        ...
        while True:
            ...
            for highlight in highlights_videos.get('results') or []:
                if highlight_id and highlight.get(
                        'clip_no') != int(highlight_id):
                    continue
                yield merge_dicts({
