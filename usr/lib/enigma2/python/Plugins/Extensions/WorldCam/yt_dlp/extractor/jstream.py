...
class JStreamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        host, publisher, mid, video_id = self._match_valid_url(
            url).group('host', 'publisher', 'mid', 'id')
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for m in re.finditer(
            r'(?s)PlayerFactoryIF\.create\(\s*({[^\}]+?})\s*\)\s*;',
                webpage):
            # TODO: using json.loads here as InfoExtractor._parse_json is not
            # classmethod
