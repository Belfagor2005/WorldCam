...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    _TESTS = [{
        ...
        # from
        # https://nitter.pussthecat.org/YouTube/status/1360363141947944964#m
        ...
        # controversial video, only works with bpctr when authenticated with
        # cookies
        ...
        # Skip download of additional client configs (remix client config in
        # this case)
        'url': 'https://music.youtube.com/watch?v=MgNrAu2pzNs',
        'only_matching': True,
        ...
        # Ensure video upload_date is in UTC timezone (video was uploaded
        # 1641170939)
        ...
        # date text is premiered video, ensure upload date in UTC (published
        # 1641172509)
        ...
        # Upload date was 2022-07-12T05:12:29-07:00, while stream start is
        # 2022-07-12T15:59:30+00:00
    ...
    _INVERSE_PLAYER_JS_VARIANT_MAP = {
        v: k for k, v in _PLAYER_JS_VARIANT_MAP.items()}
    ...
    def _live_dash_fragments(
        ...
        lack_early_segments = download_start_time - \
            (live_start_time or download_start_time) > MAX_DURATION
        ...
        def _extract_sequence_from_mpd(refresh_sequence, immediate):
            ...
            expire_fast = immediate or (last_error and isinstance(
                last_error, HTTPError) and last_error.status == 403)
            mpd_url, stream_number, is_live = (
                mpd_feed(
                    format_id, 5 if expire_fast else 18000) or (
                    mpd_url, stream_number, False))
            ...
            _last_seq = int(re.search(r'(?:/|^)sq/(\d+)',
                            fragments[-1]['path']).group(1))
        ...
        while is_live:
            ...
            if last_segment_url:
                ...
                try:
                    ...
                    urlh = None
                last_seq = try_get(
                    urlh, lambda x: int_or_none(
                        x.headers['X-Head-Seqnum']))
                if last_seq is None:
                    no_fragment_score += 2
            ...
            if lack_early_segments:
                known_idx = max(known_idx, last_seq -
                                int(MAX_DURATION // fragments[-1]['duration']))
            try:
                for idx in range(known_idx, last_seq):
                    # do not update sequence here or you'll get skipped some
                    # part of it
    ...
    def _construct_player_url(self, *, player_id=None, player_url=None):
        ...
        variant = self._configuration_arg('player_js_variant', [''])[
            0] or self._DEFAULT_PLAYER_JS_VARIANT
        if variant not in (*self._PLAYER_JS_VARIANT_MAP, 'actual'):
            self.report_warning(
                f'Invalid player JS variant name "{variant}" requested. ' f'Valid choices are: {
                    ", ".join(
                        self._PLAYER_JS_VARIANT_MAP)}',
                only_once=True)
        ...
        if not player_url:
            ...
            if variant == 'actual':
                # We don't have an actual variant so we always use 'main' &
                # don't need to write debug
                variant = 'main'
            return urljoin('https://www.youtube.com',
                           f'/s/player/{player_id}/{self._PLAYER_JS_VARIANT_MAP[variant]}')

        actual_variant = self._get_player_id_variant_and_path(player_url)[1]
        if not force_player_id and (
                variant == 'actual' or variant == actual_variant):
        ...
        if variant == 'actual':
            if actual_variant:
                ...
                # We need to force player_id but can't determine variant; fall
                # back to 'main' variant
                variant = 'main'
    ...
    def _download_player_url(self, video_id, fatal=False):
        if player_id_override := self._get_player_js_version()[1]:
            self.write_debug(
                f'Forcing player {player_id_override}',
                only_once=True)
    ...
    def _get_player_id_variant_and_path(self, player_url):
        ...
        player_path = remove_start(
            urllib.parse.urlparse(player_url).path,
            f'/s/player/{player_id}/')
        variant = self._INVERSE_PLAYER_JS_VARIANT_MAP.get(player_path) or next(
            (v for k, v in self._INVERSE_PLAYER_JS_VARIANT_MAP.items() if re.fullmatch(
                re.escape(k).replace(
                    'en_US', r'[a-zA-Z0-9_]+'), player_path)), None)
        if not variant:
            self.write_debug(
    ...
    def _sig_spec_cache_id(self, player_url, spec_id):
        return join_nonempty(
            self._player_js_cache_key(player_url),
            str(spec_id))
    ...
    def _extract_signature_timestamp(
        ...
        if CACHE_ENABLED and (
            sts := self._load_player_data_from_cache(
                'sts',
                player_url)):
            return sts
    ...
    def _extract_from_webpage(cls, url, webpage):
        ...
        for id_ in re.findall(
            r'class="lazyYT" data-youtube-id="([^"]+)"',
                webpage):
    ...
    def _extract_chapters_from_engagement_panel(self, data, duration):
        content_list = traverse_obj(
            ...
            ('engagementPanels',
             ...
             'contents'),
            expected_type=list)
        def chapter_time(chapter): return parse_duration(
            self._get_text(chapter, 'timeDescription'))

        def chapter_title(chapter): return self._get_text(chapter, 'title')
        ...
        return next(
            filter(
                ...
                (self._extract_chapters_helper(
                    ...
                    chapter_time,
                    chapter_title,
                    duration) for contents in content_list)),
            [])
    ...
    def _extract_comment(self, entities, parent=None):
        ...
        return {
            ...
            **traverse_obj(comment_entity_payload, {
            ...
            # FIXME: non-standard, but we need a way of showing that it is an
            # estimate.
    ...
    def _extract_comment_old(self, comment_renderer, parent=None):
        ...
        # Timestamp is an estimate calculated from the current time and
        # time_text
    ...
    def _comment_entries(
        ...
        def get_single_config_arg(
            c): return self._configuration_arg(c, [''])[0]
        ...
        def extract_thread(contents, entity_payloads):
            ...
            for content in contents:
                ...
                # new comment format
                    ...
                    if comment:
                        comment['is_pinned'] = traverse_obj(
                            view_model, ('pinnedText', {str})) is not None
        ...
        if is_first_continuation and not continuation:
            ...
            # required check_get_keys missing. So we will disable that check
        ...
        for page_num in itertools.count(0):
            ...
            mutations = traverse_obj(
                response, ('frameworkUpdates', 'entityBatchUpdate', 'mutations', ..., {dict}))
        ...
        message = self._get_text(
            root_continuation_data, ('contents', ..., 'messageRenderer', 'text'), max_runs=1)
        if message and not parent and tracker['running_total'] == 0:
            self.report_warning(
                f'Youtube said: {message}',
                video_id=video_id,
                only_once=True)
            ...
            try:
                return base64.urlsafe_b64encode(
                    base64.urlsafe_b64decode(
                        urllib.parse.unquote(po_token))).decode()
    ...
    def fetch_po_token(
        ...
        experiments = traverse_obj(
            ytcfg, ('WEB_PLAYER_CONTEXT_CONFIGS', ..., 'serializedExperimentFlags', {
        ...
        if config_po_token:
            # GVS WebPO token is bound to data_sync_id / account Session ID
            # when logged in.
            if (
                player_url and context == _PoTokenContext.GVS
    ...
    def _fetch_po_token(self, client, **kwargs):
        ...
        fetch_pot_policy = self._configuration_arg(
            'fetch_pot', [''], ie_key=YoutubeIE)[0]
        ...
        pot_request = PoTokenRequest(
            ...
            innertube_context=traverse_obj(
                kwargs, ('ytcfg', 'INNERTUBE_CONTEXT')),
    ...
    def _extract_player_response(
        ...
        default_pp = traverse_obj(
            INNERTUBE_CLIENTS,
            (_split_innertube_client(client)[0],
             'PLAYER_PARAMS',
             {str}))
        if player_params := self._configuration_arg(
                'player_params', [default_pp], casesense=True)[0]:
        ...
        sts = self._extract_signature_timestamp(
            video_id, player_url, webpage_ytcfg, fatal=False) if player_url else None
    ...
    def _get_requested_clients(
        ...
        requested_clients = []
        excluded_clients = []
        js_runtime_available = any(p.is_available()
                                   for p in self._jsc_director.providers.values())
        default_clients = (
        ...
        if not (
                requested_clients or excluded_clients) and default_clients == self._DEFAULT_JSLESS_CLIENTS:
            self.report_warning(
        ...
        if self.is_authenticated:
            if (smuggled_data.get('is_music_url') or self.is_music_url(
                    url)) and 'web_music' not in requested_clients:
    ...
    def _extract_player_responses(
        ...
        while clients:
            ...
            if 'configs' not in self._configuration_arg(
                    'player_skip') and client != webpage_client:
                player_ytcfg = self._download_ytcfg(
                    client, video_id) or player_ytcfg

            player_url = player_url or self._extract_player_url(
                webpage_ytcfg, player_ytcfg, webpage=webpage)
            require_js_player = self._get_default_ytcfg(
                client).get('REQUIRE_JS_PLAYER')
            ...
            pr = None
            if client == webpage_client and 'player_response' not in self._configuration_arg(
                    'webpage_skip'):
                pr = initial_pr
            ...
            # Don't need a player PO token for WEB if using player response
            # from webpage
            ...
            if pr_id := self._invalid_player_response(pr, video_id):
                ...
                sd[STREAMING_DATA_PLAYER_TOKEN_PROVIDED] = bool(
                    player_po_token)
                ...
                for f in traverse_obj(
                    ...
                    f[STREAMING_DATA_PLAYER_TOKEN_PROVIDED] = bool(
                        player_po_token)
                if deprioritize_pr:
            ...
            # Unauthenticated users will only get web_embedded client formats
            # if age-gated
            ...
            # If account is not age-verified, _is_agegated() will be truthy for
            # non-embedded clients
    ...
    def _extract_formats_and_subtitles(
        ...
        def gvs_pot_required(policy, is_premium_subscriber, has_player_token):
            return (
                policy.required and not (
                    policy.not_required_with_player_token and has_player_token) and not (
                    policy.not_required_for_premium and is_premium_subscriber))
        ...
        for pr in player_responses:
            ...
            def get_stream_id(fmt_stream):
                return str_or_none(fmt_stream.get('itag')), traverse_obj(
                    fmt_stream, 'audioTrack', 'id'), fmt_stream.get('isDrc')
            ...
            def process_format_stream(fmt_stream, proto, missing_pot):
                ...
                if quality == 'tiny' or not quality:
                    quality = fmt_stream.get(
                        'audioQuality', '').lower() or quality
                ...
                dct = {
                    ...
                    'format_note': join_nonempty(
                        ...
                        try_get(
                            fmt_stream, lambda x: x['projectionType'].replace(
                                'RECTANGULAR', '').lower()),
                        try_get(
                            fmt_stream, lambda x: x['spatialAudioType'].replace(
                                'SPATIAL_AUDIO_TYPE_', '').lower()),
                        is_damaged and 'DAMAGED', missing_pot and 'MISSING POT',
                        (self.get_param('verbose')
                         or all_formats) and short_client_name(client_name),
                        delim=', '),
            ...
            def process_https_formats():
                ...
                for fmt_stream in streaming_formats:
                    ...
                    # number of fragment that would subsequently requested with
                    # (`&sq=N`)
                    ...
                    pot_policy: GvsPoTokenPolicy = self._get_default_ytcfg(
                        client_name)['GVS_PO_TOKEN_POLICY'][StreamingProtocol.HTTPS]
                    ...
                    if not fmt_url:
                        ...
                        sc = urllib.parse.parse_qs(
                            fmt_stream.get('signatureCipher'))
                        fmt_url = url_or_none(
                            try_get(sc, lambda x: x['url'][0]))
                    ...
                    if encrypted_sig:
                        ...
                        if spec:
                            self.write_debug(
                                f'Using cached signature function {spec_cache_id}', only_once=True)
                            fmt_url += '&{}={}'.format(traverse_obj(
                                sc, ('sp', -1)) or 'signature', solve_sig(encrypted_sig, spec))
                        else:
                    ...
                    # For handling potential pre-playback required waiting
                    # period
                ...
                # Retrieve all JSC Sig and n requests for this player response
                # in one go
                n_challenges = {}
                s_challenges = {}
                for fmt in https_fmts:
                    ...
                    if s_challenge is not None:
                        s_challenges.setdefault(
                            len(s_challenge), {}).setdefault(
                            s_challenge, []).append(fmt)
                            ...
                            input=SigChallengeInput(
                                challenges=[
                                    ''.join(
                                        map(
                                            chr,
                                            range(spec_id))) for spec_id in s_challenges],
                                player_url=player_url)))

                if challenge_requests:
                    for _challenge_request, challenge_response in self._jsc_director.bulk_solve(
                            challenge_requests):
                    ...
                    for cfmts in list(s_challenges.values()) + \
                            list(n_challenges.values()):
                        for fmt in cfmts:
                ...
                for fmt in https_fmts:
                    if (
                            all_formats or 'dashy' in format_types) and fmt['filesize']:
                        yield {
                            **fmt,
            ...
            def process_manifest_format(
                    f, proto, client_name, itag, missing_pot):
                ...
                # Deprioritize since its pre-merged m3u8 formats may have lower
                ...
                f['quality'] = q(
                    itag_qualities.get(
                        try_get(
                            f, lambda f: f['format_id'].split('-')[0]), -1))
                if f['quality'] == -1 and f.get('height'):
                    f['quality'] = q(
                        res_qualities[min(res_qualities, key=lambda x: abs(x - f['height']))])
            ...
            if hls_manifest_url:
                ...
                require_po_token = gvs_pot_required(
                    pot_policy, is_premium_subscriber, player_token_provided)
                po_token = gvs_pots.get(client_name, fetch_po_token_func(
                    required=require_po_token or pot_policy.recommended))
                if po_token:
                    hls_manifest_url = hls_manifest_url.rstrip(
                        '/') + f'/pot/{po_token}'
                    if client_name not in gvs_pots:
                        gvs_pots[client_name] = po_token
                if require_po_token and not po_token and 'missing_pot' not in self._configuration_arg(
                        'formats'):
                    self._report_pot_format_skipped(
                        video_id, client_name, 'hls')
                else:
            ...
            if dash_manifest_url:
                ...
                po_token = gvs_pots.get(client_name, fetch_po_token_func(
                    required=require_po_token or pot_policy.recommended))
                if po_token:
                    dash_manifest_url = dash_manifest_url.rstrip(
                        '/') + f'/pot/{po_token}'
                    if client_name not in gvs_pots:
                        gvs_pots[client_name] = po_token
                if require_po_token and not po_token and 'missing_pot' not in self._configuration_arg(
                    ...
                    subtitles = self._merge_subtitles(
                        subs, subtitles)  # Prioritize HLS subs over DASH
                    for f in formats:
                        if process_manifest_format(
                            f,
                            'dash',
    ...
    def _extract_storyboard(self, player_responses, duration):
        ...
        for i, args in enumerate(spec):
            ...
            url = base_url.replace(
                '$L', str(L - i)).replace('$N', N) + f'&sigh={sigh}'
    ...
    def _download_initial_webpage(self, webpage_url, webpage_client, video_id):
        webpage = None
        if webpage_url and 'webpage' not in self._configuration_arg(
                'player_skip'):
            ...
            pp = (
                self._configuration_arg(
                    'player_params',
                    [None],
                    casesense=True)[0] or traverse_obj(
                    INNERTUBE_CLIENTS,
                    (webpage_client,
                     'PLAYER_PARAMS',
                     {str})))
    ...
    def _list_formats(
        ...
        *formats, subtitles = self._extract_formats_and_subtitles(
        ...
        if all(f.get('has_drm') for f in formats):
            # If there are no formats that definitely don't have DRM, all have
            # DRM
            for f in formats:
                f['has_drm'] = True
    ...
    def _download_initial_data(
            ...
            webpage_ytcfg):
        initial_data = None
        if webpage and 'initial_data' not in self._configuration_arg(
                'webpage_skip'):
    ...
    def _initial_extract(
        ...
        webpage = self._download_initial_webpage(
            webpage_url, webpage_client, video_id)
        webpage_ytcfg = self.extract_ytcfg(
            video_id, webpage) or self._get_default_ytcfg(webpage_client)
    ...
    def _real_extract(self, url):
        ...
        # videoDescriptionHeaderRenderer also has
        # publishDate/channel/handle/ucid, but not needed
        ...
        # List of possible thumbnails - Ref:
        ...
        for thumb in thumbnails:
            i = next((i for i, t in enumerate(thumbnail_names)
                     if f'/{video_id}/{t}' in thumb['url']), n_thumbnail_names)
            thumb['preference'] = (
                0 if '.webp' in thumb['url'] else -1) - (2 * i)
        ...
        info = {
            ...
            # URL checking if user don't care about getting the best possible
            # thumbnail
            ...
            '_format_sort_fields': (  # source_preference is lower for potentially damaged formats
                ...
                'vcodec:vp9.2' if 'prefer-vp9-sort' in self.get_param(
                    'compat_opts', []) else 'vcodec',
        ...
        # Only web/mweb clients provide translationLanguages, so include
        # initial_pr in the traversal
        translation_languages = {
            lang['languageCode']: self._get_text(
                lang['languageName'],
                max_runs=1) for lang in traverse_obj(
                player_responses,
                (...,
                 'captions',
                 ...
                 lambda _,
                 v: v['languageCode'] and v['languageName']))}
        # NB: Constructing the full subtitle dictionary is slow
        get_translated_subs = 'translated_subs' not in self._configuration_arg('skip') and (
            self.get_param('writeautomaticsub', False) or self.get_param('listsubtitles'))

        # Filter out initial_pr which does not have streamingData (smuggled
        # client context)
        prs = traverse_obj(
            player_responses,
            (lambda _,
             v: v['streamingData'] and v['captions']['playerCaptionsTracklistRenderer']))
        all_captions = traverse_obj(
            prs, (..., 'captions', 'playerCaptionsTracklistRenderer', 'captionTracks', ..., {dict}))
        need_subs_langs = {get_lang_code(
            sub) for sub in all_captions if sub.get('kind') != 'asr'}
        need_caps_langs = {
        ...
        for pr in prs:
            ...
            for caption_track in traverse_obj(
                ...
                for trans_code, trans_name in translation_languages.items():
                    ...
                    if caption_track.get(
                            'kind') != 'asr' and trans_code != 'und':
                    ...
                    if lang_code == f'a-{orig_trans_code}':
                        ...
                        # The subs are returned without "-orig" as well for
                        # compatibility
                        process_language(
                            automatic_captions,
                            base_url,
                    ...
                    process_language(
                        ...
                        trans_name,
                        client_name,
                        pot_params if orig_lang == orig_trans_code else {
                            'tlang': trans_code,
                            **pot_params})
        ...
        info['comment_count'] = traverse_obj(
            ...
            ('engagementPanels',
             lambda _,
             v: v['engagementPanelSectionListRenderer']['panelIdentifier'] in COMMENTS_SECTION_IDS,
                'engagementPanelSectionListRenderer',
                'header',
        ...
        if initial_data:
            info['chapters'] = (
                self._extract_chapters_from_json(
                    initial_data,
                    duration) or self._extract_chapters_from_engagement_panel(
                    initial_data,
                    duration) or self._extract_chapters_from_description(
                    video_description,
                    duration) or None)
        ...
        if vpir:
            ...
            if vcr:
                ...
                # Upcoming premieres with waiting count are treated as live
                # here
                if vcr.get('isLive'):
        ...
        if vsir:
            ...
            for row in rows:
                if try_get(
                        row,
                        lambda x: x['metadataRowRenderer']['hasDividerLine']) is True:
                    multiple_songs = True
                    break
            for row in rows:
            ...
            owner_badges = self._extract_badges(traverse_obj(
                vsir, ('owner', 'videoOwnerRenderer', 'badges')))
            if self._has_badge(owner_badges, BadgeType.VERIFIED):
                info['channel_is_verified'] = True

        info.update({'uploader': info.get('channel'),
                     'uploader_id': channel_handle,
                     'uploader_url': format_field(channel_handle,
                                                  None,
        ...
        # Currently the uploadDate in microformats appears to be in US/Pacific
        # timezone.
        timestamp = (
            parse_iso8601(
                get_first(
                    microformats,
                    'uploadDate'),
                timezone=NO_DEFAULT) or parse_iso8601(
                search_meta('uploadDate'),
                timezone=NO_DEFAULT))
        upload_date = (
            dt.datetime.fromtimestamp(
                timestamp,
                dt.timezone.utc).strftime('%Y%m%d') if timestamp else (
                unified_strdate(
                    get_first(
                        microformats,
                        'uploadDate')) or unified_strdate(
                    search_meta('uploadDate'))))
        ...
        # See:
        # https://github.com/yt-dlp/yt-dlp/pull/2223#issuecomment-1008485139
        if not upload_date or (
            not timestamp and live_status in (
                'not_live',
                None)):
            # this should be in UTC, as configured in the cookie/client context
            upload_date = strftime_or_none(self._parse_time_text(
                self._get_text(vpir, 'dateText'))) or upload_date

        info['upload_date'] = upload_date
        info['timestamp'] = timestamp

        if upload_date and live_status not in (
                'is_live', 'post_live', 'is_upcoming'):
            # Newly uploaded videos' HLS formats are potentially problematic
            # and need to be checked
            upload_datetime = datetime_from_str(
                upload_date).replace(tzinfo=dt.timezone.utc)
        ...
        is_private = (
            self._has_badge(
                badges,
                BadgeType.AVAILABILITY_PRIVATE) or get_first(
                video_details,
                'isPrivate',
                expected_type=bool))
