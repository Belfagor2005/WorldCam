...
class GameJoltBaseIE(InfoExtractor):
    ...
    def _parse_content_as_text(self, content):
        ...
        for outer_content in outer_contents:
            if outer_content.get('type') != 'paragraph':
                ...
                continue
            inner_contents, inner_content_text = outer_content.get(
                'content') or [], ''
    ...
    def _get_comments(self, post_num_id, post_hash_id):
        sort_by, scroll_id = self._configuration_arg(
            'comment_sort', ['hot'], ie_key=GameJoltIE.ie_key())[0], -1
    ...
    def _parse_post(self, post_data):
        ...
        lead_content = self._parse_json(
            post_data.get('lead_content') or '{}',
            post_id,
            fatal=False) or {}
        ...
        if post_data.get('has_article'):
            article_content = self._parse_json(
                post_data.get('article_content') or self._call_api(
        ...
        video_data = traverse_obj(
            post_data, ('videos', ...), expected_type=dict, get_all=False) or {}
        formats, subtitles, thumbnails = [], {}, []
        for media in video_data.get('media') or []:
            media_url, mimetype, ext, media_id = media['img_url'], media.get(
                'filetype', ''), determine_ext(media['img_url']), media.get('type')
        ...
        for media in post_data.get('media', []):
            if determine_ext(
                    media['img_url']) != 'gif' or 'gif' not in media.get(
                    'filetype',
                    ''):
                continue
            gif_entries.append(
                    ...
                    'formats': [
                        {
                            ...
                            'mediaserver_url',
                            'mediaserver_url_mp4',
                            'mediaserver_url_webm') if media.get(url_key)],
                })
        if gif_entries:
            return {
                '_type': 'playlist',
...
class GameJoltPostListBaseIE(GameJoltBaseIE):
    def _entries(
        ...
        items = initial_items or self._call_api(
            endpoint, list_id, note=note, errnote=errnote)['items']
        while items:
            for item in items:
                yield self._parse_post(item['action_resource_model'])
            scroll_id = items[-1]['scroll_id']
            page_num += 1
            items = self._call_api(endpoint,
                                   list_id,
                                   note=f'{note} page {page_num}',
                                   errnote=errnote,
                                   data=json.dumps({'scrollDirection': 'from',
                                                    'scrollId': scroll_id,
                                                    }).encode()).get('items')
...
class GameJoltUserIE(GameJoltPostListBaseIE):
    ...
    _TESTS = [{'url': 'https://gamejolt.com/@BlazikenSuperStar',
               'playlist_mincount': 1,
               'info_dict': {'id': '6116784',
                             'title': 'S. Blaze',
    ...
    def _real_extract(self, url):
        ...
        user_data = self._call_api(
            ...
            errnote='Unable to download user info')['user']
...
class GameJoltGameIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        game_data = self._call_api(
            ...
            errnote='Unable to download game info')['game']
...
class GameJoltCommunityIE(GameJoltPostListBaseIE):
    ...
    _TESTS = [{'url': 'https://gamejolt.com/c/fnf/videos',
               'playlist_mincount': 50,
               'info_dict': {'id': 'fnf/videos',
                             'title': 'Friday Night Funkin\' - Videos',
                             'description': 'md5:6d8c06f27460f7d35c1554757ffe53c8',
                             },
               'params': {'playlistend': 50,
                          'ignore_no_formats_error': True,
                          },
               'expected_warnings': ['skipping format',
                                     'No video formats found',
              ...
              {'url': 'https://gamejolt.com/c/youtubers',
               'playlist_mincount': 50,
               'info_dict': {'id': 'youtubers/featured',
    ...
    def _real_extract(self, url):
        display_id, community_id, channel_id, sort_by = self._match_valid_url(
            url).group('id', 'community', 'channel', 'sort')
        ...
        community_data = self._call_api(
            ...
            errnote='Unable to download community info')['community']
...
class GameJoltSearchIE(GameJoltPostListBaseIE):
    ...
    _TESTS = [{'url': 'https://gamejolt.com/search?foo=bar&q=%23fnf',
               'playlist_mincount': 50,
               'info_dict': {'id': '#fnf',
                             'title': '#fnf',
                             },
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._search_entries(
                query,
                filter_mode,
                display_query) if filter_mode else self._entries(
                ...
                initial_items=self._call_api(
                    ...
                    errnote='Unable to download initial post list')['posts']),
            display_query,
            display_query)
