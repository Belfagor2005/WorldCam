...
class SponsorBlockPP(FFmpegPostProcessor):
    ...
    def _get_sponsor_chapters(self, info, duration):
        ...
        def duration_filter(s):
            ...
            # SponsorBlock duration may be absent or it may deviate from the
            # real one.
            diff = abs(duration - s['videoDuration']
                       ) if s['videoDuration'] else 0
            return diff < 1 or (diff < 5 and diff /
                                (start_end[1] - start_end[0]) < 0.05)
