...
class ARDBetaMediathekIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://www.ardmediathek.de/video/filme-im-mdr/liebe-auf-vier-pfoten/mdr-fernsehen/Y3JpZDovL21kci5kZS9zZW5kdW5nLzI4MjA0MC80MjIwOTEtNDAyNTM0',
               'md5': 'b6e8ab03f2bcc6e1f9e6cef25fcc03c4',
               'info_dict': {'display_id': 'Y3JpZDovL21kci5kZS9zZW5kdW5nLzI4MjA0MC80MjIwOTEtNDAyNTM0',
                             'id': '12939099',
    ...
    def _extract_episode_info(self, title):
        patterns = [
            ...
            # from:
            # https://www.ardmediathek.de/one/sendung/doctor-who/Y3JpZDovL3dkci5kZS9vbmUvZG9jdG9yIHdobw
            r'.*(?P<ep_info> \(S(?P<season_number>\d+)/E(?P<episode_number>\d+)\)).*',
            # E.g.: title="Fritjof aus Norwegen (2) (AD)"
            # from:
            # https://www.ardmediathek.de/ard/sammlung/der-krieg-und-ich/68cMkqJdllm639Skj4c7sS/
        ...
        return traverse_obj(
            patterns, (..., {
                functools.partial(
                    re.match, string=title)}, {
                'season_number': (
                    'season_number', {int_or_none}), 'episode_number': (
                        'episode_number', {int_or_none}), 'episode': (
                            (('episode', {str_or_none}), ('ep_info', {
                                lambda x: title.replace(
                                    x, '')}), ('title', {str}), ), {
                                        str.strip}), }), get_all=False)
    ...
    def _real_extract(self, url):
        ...
        # Ref:
        # https://github.com/yt-dlp/yt-dlp/issues/8731#issuecomment-1874398283
        old_id = traverse_obj(
        ...
        for stream in traverse_obj(media_data, ('streams', ..., {dict})):
            ...
            for media in traverse_obj(
                ...
                audio_kind = traverse_obj(
                    media, ('audios', 0, 'kind', {str}), default='').replace(
                    'standard', '')
                lang_code = traverse_obj(
                    media, ('audios', 0, 'languageCode', {str})) or 'deu'
        ...
        return {'id': video_id,
                ...
                **traverse_obj(media_data,
                               {'chapters': ('pluginData',
                                             ...
                                             'chapterArray',
                                             lambda _,
                                             v: int_or_none(v['chapterTime']),
                                             {'start_time': ('chapterTime',
                                                             {int_or_none}),
                                              'title': ('chapterTitle',
                                                        {str}),
                                              }),
                ...
                **traverse_obj(media_data,
                               ('meta',
                                {'title': 'title',
                                 ...
                                 'timestamp': ('broadcastedOnDateTime',
                                               {parse_iso8601}),
                                 'series': 'seriesTitle',
                                 'thumbnail': ('images',
                                               0,
...
class ARDMediathekCollectionIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        playlist_id, display_id, playlist_type, season_number, version = self._match_valid_url(
            url).group('id', 'display_id', 'playlist', 'season', 'version')
        ...
        def fetch_page(page_num):
            for item in traverse_obj(
                ...
                if not item_id or item_id == playlist_id:
                    continue
                item_mode = 'sammlung' if item.get(
                    'type') == 'compilation' else 'video'
...
class ARDAudiothekPlaylistIE(ARDAudiothekBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries, urn, display_id=playlist, **traverse_obj(
                playlist_info, {
                    'title': (
