...
class YoutubeDL:
    ...
    def process_info(self, info_dict):
        ...
        if annofn:
            ...
            if not self.params.get(
                'overwrites',
                    True) and os.path.exists(annofn):
            ...
            try:
                ...
                def existing_video_file(*filepaths):
                    ...
                    file = self.existing_file(itertools.chain(
                        *zip(map(converted, filepaths), filepaths)), default_overwrite=False)
                    if file:
    ...
    def print_debug_header(self):
        ...
        def get_encoding(stream):
            ret = str(
                getattr(
                    stream,
                    'encoding',
                    f'missing ({type(stream).__name__})'))
            additional_info = []
        ...
        for plugin_type, plugins in (
            ...
            if plugin_type == 'Extractor':
                display_list.extend(
                    f'{plugins[-1].IE_NAME.partition("+")[2]} ({parent.__name__})' for parent,
                    plugins in plugin_ies_overrides.value.items())
            if not display_list:
                continue
            write_debug(
    ...
    def cookiejar(self):
        ...
        try:
            ...
            # compat: <=py3.9: `traceback.format_exception` has a different
            # signature
            self.report_error(
                str(cause), tb=''.join(
