...
class RedirectHandler(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        new_headers = {
            k: v for k,
            v in req.headers.items() if k.title() not in remove_headers}
...
class UrllibResponseAdapter(Response):
    ...
    def __init__(self, res: http.client.HTTPResponse |
        ...
        # 2.
        # https://docs.python.org/3.10/library/http.client.html#http.client.HTTPResponse.status
        super().__init__(
            fp=res, headers=res.headers, url=res.url, status=getattr(
                res, 'status', None) or res.getcode(), reason=getattr(
                res, 'reason', None))
    ...
    def read(self, amt=None):
        try:
            ...
            if isinstance(
                    self.fp,
                    http.client.HTTPResponse) and underlying is None:
                # http.client.HTTPResponse automatically closes itself when
                # fully read
                self.close()
            elif isinstance(self.fp, urllib.response.addinfourl) and underlying is not None:
                # urllib's addinfourl does not close the underlying fp
                # automatically when fully read
                if isinstance(underlying, io.BytesIO):
                    # data URLs or in-memory responses (e.g.
                    # gzip/deflate/brotli decoded)
                    if underlying.tell() >= len(underlying.getbuffer()):
                        self.close()
                elif isinstance(underlying, io.BufferedReader) and amt is None:
                    # file URLs.
                    # XXX: this will not mark the response as closed if it was
                    # fully read with amt.
                    self.close()
    ...
    def __init__(self, *, enable_file_urls: bool = False, **kwargs):
        ...
        if self.enable_file_urls:
            self._SUPPORTED_URL_SCHEMES = (
                *self._SUPPORTED_URL_SCHEMES, 'file')
        ...
        try:
            ...
            if isinstance(
                ...
                (http.client.HTTPResponse,
                 urllib.response.addinfourl)):
                # Prevent file object from being closed when
                # urllib.error.HTTPError is destroyed.
                e._closer.close_called = True
                raise HTTPError(UrllibResponseAdapter(e.fp),
                                redirect_loop='redirect error' in str(e)) from e
            raise  # unexpected
            ...
            if 'tunnel connection failed' in str(
                    cause).lower() or isinstance(cause, SocksProxyError):
