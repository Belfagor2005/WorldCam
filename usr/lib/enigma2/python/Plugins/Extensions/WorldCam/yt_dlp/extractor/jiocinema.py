        ...
        if token_type in ('access', 'all'):
            self.cache.store(
                JioCinemaBaseIE._NETRC_MACHINE, f'{
                    JioCinemaBaseIE._DEVICE_ID}-access', JioCinemaBaseIE._ACCESS_TOKEN)
        if token_type in ('refresh', 'all'):
            self.cache.store(JioCinemaBaseIE._NETRC_MACHINE,
    ...
    def _perform_login(self, username, password):
        ...
        if JioCinemaBaseIE._REFRESH_TOKEN and username != 'device':
            ...
            if JioCinemaBaseIE._REFRESH_TOKEN:
                self._cache_token('access')
        self.to_screen(
            f'Logging in as device ID "{
                JioCinemaBaseIE._DEVICE_ID}"')
