...
class OpenRecBaseIE(InfoExtractor):
    ...
    def _extract_movie(self, webpage, video_id, name, is_live):
        ...
        movie_stores = [
            # extract all three important data (most of data are duplicated
            # each other, but slightly different!)
            traverse_obj(
                window_stores,
                ('v8',
                 'state',
        ...
        return {
            'id': video_id, 'title': get_first(
                movie_stores, 'title'), 'description': get_first(
                movie_stores, 'introduction'), 'thumbnail': get_first(
                movie_stores, 'thumbnailUrl'), 'formats': formats, 'uploader': get_first(
                    movie_stores, ('channel', 'user', 'name')), 'uploader_id': get_first(
                        movie_stores, ('channel', 'user', 'id')), 'timestamp': int_or_none(
                            get_first(
                                movie_stores, [
                                    'publishedAt', 'time']), scale=1000) or unified_timestamp(
                                        get_first(
                                            movie_stores, 'publishedAt')), 'is_live': is_live, 'http_headers': self._M3U8_HEADERS, }
