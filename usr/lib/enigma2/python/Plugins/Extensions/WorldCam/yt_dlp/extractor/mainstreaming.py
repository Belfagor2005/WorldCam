...
class MainStreamingIE(InfoExtractor):
    ...
    def _get_webtools_host(host):
        if not host.startswith('webtools'):
            host = 'webtools' + \
                ('-' if not host.startswith('.') else '') + host
        return host
    ...
    def _call_webtools_api(
        ...
        return self._download_json(
            urljoin(
                self._get_webtools_base_url(host),
                path),
            item_id,
            query=query,
            note=note,
    ...
    def _real_extract(self, url):
        ...
        if content_type == 20:
            ...
            heartbeat = self._call_api(
                ...
                video_id,
                note='Checking stream status') or {}
            if heartbeat.get('heartBeatUp') is False:
                self.raise_no_formats(
                    f'MainStreaming said: {
                        heartbeat.get("responseMessage")}',
                    expected=True)
                live_status = 'was_live'
        ...
        # Playlist
            ...
            return self.playlist_result(
                self._playlist_entries(
                    host,
                    content_info.get('playlistContents')),
                video_id,
                title,
                description)
        # Normal video content?
            ...
            formats.append({'url': format_base_url % 'original.mp4',
                           'format_note': 'original', 'quality': 1})
        else:
        ...
        return {
            ...
            'duration': parse_duration(
                content_info.get('duration')),
            'tags': content_info.get('tags'),
            'subtitles': subtitles,
            'thumbnail': urljoin(
                self._get_webtools_base_url(host),
                f'image/{video_id}/poster'),
        }
