...
class TeamcocoIE(TeamcocoBaseIE):
    ...
    _TESTS = [{'url': 'http://teamcoco.com/video/mary-kay-remote',
               'info_dict': {'id': '80187',
    ...
    def _real_extract(self, url):
        ...
        thumbnail = traverse_obj(
            info, (('image', 'poster'), {
                urljoin('https://teamcoco.com/')}), get_all=False)
    ...
    def _real_extract(self, url):
        ...
        if not video_id:
            self.raise_no_formats(
                'Unable to extract video ID from webpage',
                expected=True)
