...
class TubiTvIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://tubitv.com/movies/100004539/the-39-steps',
               'info_dict': {'id': '100004539',
                             'ext': 'mp4',
                             'title': 'The 39 Steps',
    ...
    def _real_extract(self, url):
        ...
        season_number, episode_number, episode_title = self._search_regex(
            r'^S(\d+):E(\d+) - (.+)', title, 'episode info', fatal=False, group=(
                1, 2, 3), default=(
                None, None, None))
...
class TubiTvShowIE(InfoExtractor):
    ...
    def _entries(self, show_url, playlist_id, selected_season):
        ...
        # v['number'] is already a decimal string, but stringify to protect
        # against API changes
        path = [lambda _, v: str(
            v['number']) == selected_season] if selected_season else [..., {dict}]
    ...
    def _real_extract(self, url):
        playlist_id, selected_season = self._match_valid_url(
            url).group('show_name', 'season')
        if selected_season:
