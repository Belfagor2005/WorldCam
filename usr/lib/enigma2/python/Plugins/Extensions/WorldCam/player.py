...
from enigma import eServiceReference, eTimer, iPlayableService, getDesktop  # , iServiceInformation
...
class WorldCamPlayer(
	...
	def show_help_message(self):
		...
		def open_msgbox():
			self.session.open(MessageBox, help_text, MessageBox.TYPE_INFO, timeout=5)
	...
	def toggle_favorite(self):
		...
		if FavoritesManager.is_favorite(current_webcam["url"]):
			...
			message = _("Removed from favorites") if success else _("Error removing favorite")
		else:
			success = FavoritesManager.add_favorite(current_webcam["name"], current_webcam["url"])
			message = _("Added to favorites!") if success else _("Error adding favorite")
	...
	def start_playback(self):
		...
		try:
			current_webcam = self.get_current_webcam()
			self.logger.info(f"Starting playback for: {current_webcam['name']}")
			...
			if 'youtube.com' in current_webcam['url'] or 'youtu.be' in current_webcam['url']:
				self.play_youtube(current_webcam['url'], current_webcam['name'])
				return
		...
		try:
				...
				try:
					with open(cookiefile, "r") as f:
						...
						if content.count("\t") >= 6:
							...
							self.logger.warning("Invalid cookie file format, skipping")
				except Exception as e:
					self.logger.warning("Error reading cookie file: %s", str(e))
			...
			if "live stream recording is not available" in str(e):
				try:
					...
					vod_url = url.replace("/live", "")  # Rimuove il contesto live
