...
try:
	...
	# Fallback for systems without dataclasses
	sys.path.append(join(PLUGIN_PATH, "dataclasses_backport"))
	import dataclasses
...
class WorldCamPlayer(
	...
	def updateButtons(self):
		...
		# is_fav = FavoritesManager.is_favorite(current_webcam["url"])
	...
	def start_playback(self):
		...
		try:
			...
			# Handle Streamlink proxy URLs first
			if '127.0.0.1:8088/streamlink' in enhanced_url:
				self.logger.info("Processing Streamlink proxy URL")
				# Extract actual YouTube URL from proxy URL
				match = search(r'streamlink/(https?%3A%2F%2F[^&]+)', enhanced_url)
				if match:
					encoded_url = match.group(1)
					actual_url = unquote(encoded_url)
					self.logger.info(f"Extracted YouTube URL: {actual_url}")
					self.play_youtube(actual_url, current_webcam['name'])
					return
				else:
					self.logger.error("Couldn't extract URL from proxy")
		...
		try:
			...
			if "live stream recording is not available" in str(e):
				try:
					...
					self.logger.info("Trying VOD extraction method")
					vod_url = url.replace(
					...
					self.logger.info("Trying Wayback Machine archive")
	...
	def fix_streamlink_permissions(self):
		"""Ripara i permessi di Streamlink"""
		streamlink_path = "/usr/bin/streamlink"
		if not exists(streamlink_path):
			streamlink_path = "/usr/local/bin/streamlink"
		
		if exists(streamlink_path):
			try:
				# Imposta i permessi di esecuzione
				from os import stat
				chmod(streamlink_path, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
				return True
			except Exception as e:
				self.logger.error(f"Impossibile riparare i permessi di Streamlink: {str(e)}")
		return False

	def play_with_streamlink(self, video_id, title):
		""" Method using Streamlink executable """
		try:
			self.logger.info("Tentativo con Streamlink")
			
			# Ripara i permessi prima di eseguire
			self.fix_streamlink_permissions()
			...
			stream_url = subprocess.check_output(cmd, timeout=15).decode().strip()
			
			if not stream_url:
				self.logger.error("Nessun URL restituito da Streamlink")
				return False
				
			self.logger.info(f"URL estratto: {stream_url[:200]}...")
			
			# Forza l'uso di HLS se disponibile
			if '.m3u8' in stream_url:
				service_type = 5001  # HLS
				self.logger.info("Rilevato stream HLS")
			else:
				service_type = 4097  # HTTP
				self.logger.info("Rilevato stream HTTP")
			...
			import traceback
			self.logger.error(f"Errore con Streamlink: {str(e)}")
			self.logger.error(traceback.format_exc())
			return False
	...
	def play_with_streamlink_fallback(self, url):
		try:
			...
			self.logger.error("imp fallback failed: %s", str(e))
			return None
	...
	def extract_video_id(self, url):
		"""Extracts video ID from YouTube URL - handles encoded URLs"""
		try:
			# Decode URL first
			...
			patterns = [
				...
				# Add pattern for streamlink proxy URLs
				r'streamlink/https%3A%2F%2Fwww\.youtube\.com%2Fwatch%3Fv%3D([^&]+)'
			]
			...
			return super().extract_video_id(url)
