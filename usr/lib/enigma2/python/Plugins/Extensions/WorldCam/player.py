    ...
    def fix_streamlink_permissions(self):
        ...
        if exists(streamlink_path):
            try:
                ...
                from os import stat
                chmod(streamlink_path, stat.S_IRWXU | stat.S_IRGRP |
                      stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
                return True
            except Exception as e:
                self.logger.error(
                    f"Impossibile riparare i permessi di Streamlink: {str(e)}")
        return False
