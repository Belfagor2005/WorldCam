...
# fallback force folder import (issue?)
plugin_dir = dirname(abspath(__file__))
if plugin_dir not in sys.path:
	sys.path.append(plugin_dir)
sys.path.append(join(plugin_dir, 'yt_dlp'))
	...
	def start_playback(self):
		...
		try:
			...
			if self.is_direct_stream_url(self.url):
				self.logger.info("Direct stream URL detected - bypassing scraper")
				stream_url = self.url
			else:
				...
				if YoutubeDL:
					...
					with YoutubeDL(ydl_opts) as ydl:
						...
						if 'url' in info:
							...
							if formats:
								# Sort by quality: width > height > bitrate
