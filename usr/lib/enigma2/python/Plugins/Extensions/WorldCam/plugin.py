...
class WorldCamMainScreen(WebcamBaseScreen):
	...
	def on_item_selected(self):
		...
		try:
			...
			if index is not None and 0 <= index < len(self["list"].list):
				...
				if selected_item and isinstance(selected_item, list):
					...
					for category in self.categories:
						if category["name"] == selected_name:
							self.logger.info("Opening screen for category: %s" % selected_name)
							self.session.open(category["screen"])
							return
			self.logger.info("Invalid selection index or item")
		except Exception as e:
	...
	def get_categories(self):
		...
		categories = {
			"en": ["User Lists", "Continents", "Countries", "Categories", "Top Webcams"],
			"it": ["Liste Utente", "Continenti", "Paesi", "Categorie", "Webcam Top"],
			"es": ["Listas de Usuario", "Continentes", "Países", "Categorías", "Cámaras Top"],
			"de": ["Benutzerlisten", "Kontinente", "Länder", "Kategorien", "Top-Webcams"],
			"fr": ["Listes Utilisateur", "Continents", "Pays", "Catégories", "Meilleures Webcams"],
			"pl": ["Listy użytkowników", "Kontynenty", "Kraje", "Kategorie", "Najlepsze kamery"],
			"el": ["Λίστες χρηστών", "Ήπειροι", "Χώρες", "Κατηγορίες", "Κορυφαίες κάμερες"],
			"hr": ["Korisničke liste", "Kontinenti", "Zemlje", "Kategorije", "Top web kamere"],
			"sl": ["Uporabniški seznami", "Celine", "Države", "Kategorije", "Vrhunske spletne kamere"],
			"ru": ["Пользовательские списки", "Континенты", "Страны", "Категории", "Лучшие веб-камеры"],
			"zh": ["用户列表", "大洲", "国家", "类别", "热门摄像头"],
			"sq": ["Lista e përdoruesit", "Kontinentet", "Vendet", "Kategoritë", "Kamerat më të mira"],
		}
...
class WorldCamContinentScreen(WebcamBaseScreen):
	...
	def load_continents(self):
		...
		try:
			...
			# showlist(continent_names, self["list"], is_country=True)
...
class WorldCamCountryScreen(WebcamBaseScreen):
	...
	def load_countries(self):
		...
		try:
			...
			# showlist(country_names, self["list"], is_category=True)
		except Exception as e:
