...
def wcListEntry(name, idx, is_category=False, is_country=False):
	...
	# Create position parameters based on resolution
	if screen_width == 2560:
		icon_pos, icon_size = (5, 5), (60, 50)
		text_pos, text_size = (90, 0), (1200, 60)
	elif screen_width == 1920:
		icon_pos, icon_size = (5, 5), (70, 50)
		text_pos, text_size = (100, 0), (950, 50)
	else:
		icon_pos, icon_size = (3, 2), (50, 40)
...
def showlist(data, list_widget, is_category=False, is_country=False):
	"""Populate a list widget with formatted entries"""
	plist = [wcListEntry(name, idx, is_category, is_country)
			 for idx, name in enumerate(data)]

	try:
		list_widget.setList(plist)
	except Exception:
		# Fallback to direct list setting
...
class WorldCamMainScreen(WebcamBaseScreen):
	...
	def show_message(self, message, mtype=MessageBox.TYPE_INFO):
		"""Show message with delay to avoid UI conflicts"""
		timer = eTimer()
		timer.callback.append(lambda: self.session.open(MessageBox, message, mtype))
