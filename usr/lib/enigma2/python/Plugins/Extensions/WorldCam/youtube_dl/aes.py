...
if Cryptodome.AES:
    def aes_cbc_decrypt_bytes(data, key, iv):
        ...
        return Cryptodome.AES.new(
            key, Cryptodome.AES.MODE_CBC, iv).decrypt(data)
    ...
    def aes_cbc_decrypt_bytes(data, key, iv):
        ...
        return intlist_to_bytes(aes_cbc_decrypt(
            *map(bytes_to_intlist, (data, key, iv))))
    ...
    def aes_gcm_decrypt_and_verify_bytes(data, key, tag, nonce):
        ...
        return intlist_to_bytes(aes_gcm_decrypt_and_verify(
            *map(bytes_to_intlist, (data, key, tag, nonce))))


def aes_cbc_encrypt_bytes(data, key, iv, **kwargs):
    return intlist_to_bytes(aes_cbc_encrypt(
        *map(bytes_to_intlist, (data, key, iv)), **kwargs))
...
def aes_gcm_decrypt_and_verify(data, key, tag, nonce):
    ...
    if len(nonce) == 12:
        ...
        fill = (BLOCK_SIZE_BYTES - (len(nonce) %
                BLOCK_SIZE_BYTES)) % BLOCK_SIZE_BYTES + 8
        ghash_in = nonce + [0] * fill + \
            bytes_to_intlist((8 * len(nonce)).to_bytes(8, 'big'))
...
def key_expansion(data):
    ...
    while len(data) < expanded_key_size_bytes:
        ...
        for _ in range(3 if key_size_bytes ==
                       32 else 2 if key_size_bytes == 24 else 0):
            temp = data[-4:]
            data += xor(temp, data[-key_size_bytes: 4 - key_size_bytes])
...
def iter_mix_columns(data, matrix):
    for i in (0, 4, 8, 12):
        for row in matrix:
            ...
            for j in range(4):
                # xor is (+) and (-)
                mixed ^= (0 if data[i:i + 4][j] == 0 or row[j] == 0 else RIJNDAEL_EXP_TABLE[(
                    RIJNDAEL_LOG_TABLE[data[i + j]] + RIJNDAEL_LOG_TABLE[row[j]]) % 0xFF])
            yield mixed


def shift_rows(data):
    return [data[((column + row) & 0b11) * 4 + row]
