    ...
    if getattr(sys, 'frozen', False):
        ...
        if not hasattr(sys, '_MEIPASS'):
            ...
            if machine[1:] in ('x86', 'x86_64', 'amd64', 'i386', 'i686'):
                machine = '_x86' if platform.architecture()[
                    0][:2] == '32' else ''
...
def _get_system_deprecation():
    ...
    if sys.version_info < MIN_SUPPORTED:
        ...
        msg = (
            f'Support for Python version {major}.{minor} has been deprecated. '
...
class Updater:
    ...
    def cmd(self):
        ...
        # There is no sys.orig_argv in py < 3.10. Also, it can be [] when
        # frozen
    ...
    def _report_network_error(self, action, delim=';', tag=None):
        ...
        self._report_error(
            ...
            tag if tag == 'latest' else f'tag/{tag}',
            True)

    # XXX: Everything below this line in this class is deprecated / for compat
    # only
    @property
    ...
    def __getattr__(self, attribute: str):
        ...
        if attribute not in deprecated_props_map and attribute not in update_info_props_map:
            raise AttributeError(
                f'{type(self).__name__!r} object has no attribute {attribute!r}')
