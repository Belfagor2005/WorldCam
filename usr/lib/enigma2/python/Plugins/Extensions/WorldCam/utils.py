...
PY34 = version_info[0:2] >= (3, 4)
PY39 = version_info[0:2] >= (3, 9)
...
def is_ytdlp_available(logger=None):
	"""
	Check availability of yt_dlp module in different locations.
	"""
	try:
		...
		try:
			from Plugins.Extensions.WorldCam.yt_dlp import YoutubeDL
			if logger:
				logger.info("Using plugin-folder yt_dlp")
			return YoutubeDL
		except ImportError:
			try:
				from .yt_dlp import YoutubeDL
				...
				if logger:
					logger.error("yt_dlp not found locally or system-wide")
				return None
